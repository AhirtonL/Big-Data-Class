...
In this tutorial we will use the Gapminder data as example to demonstrate using regular expression in R. First, let's start off by cloning the class repository and load the Gapminder dataset into R. 
...
#Agrupando todas as ocorrências de "ee" e fazendo a busca nos dados de países.
```
...
* Start with âSouthâ
* End in âlandâ
* Have a word in its name that starts with âGaâ

```{r ex_quant, echo = FALSE}
grep("South", levels(gDat$country), value = TRUE) #Valores com "South" no início.
grep("land$", levels(gDat$country), value = TRUE) #Valores com "Land" ao final.
grep("\\Ga", levels(gDat$country), value = TRUE) #Valores com "Ga"ao final.
```
...
#Agrupando todas as ocorrências com as letras "i" ou "t" terminadas em "land" e posterior substituição por "LAND".
```
...
#Busca por valores com "o" nos dados de Continente.
```
...
```{r ex_quant, echo = FALSE}
grep("[[:punct:]]", levels(gDat$country), value = TRUE) #Valores com pontuações em seu nome.
```

Rewrite the clean.text() function that takes in a string and keeps only alpha-numeric characters, removes all spaces converts to lower case and returns it the newly formatted string. 

For example  clean.text("Coeur d'Alene") should return coeurdalene


```{r}
(string <- c("Coeur d'Alene"))
clean.text <- function(string){
    #Letras minúsculas
    temp <- tolower(string)
    # Remover tudo que não sejam números e letras
    temp <- stringr::str_replace_all(temp,"[[:punct:]]", " ")
    # Diminuindo para apenas um whitespace
